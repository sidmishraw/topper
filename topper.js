...
(function(topper) {
  ...
  topper.addTopHeader = function() {
    ...
    //
    let topperTemplateParams = workspace
      .getConfiguration(TOPPER)
      .get(TOPPER_CUSTOM_TEMPLATE_PARAMETERS);
    ...
    loginfo(
      `TopperTemplateParams from configuration: ${JSON.stringify(topperTemplateParams)}`
    );
    ...
    fetchTimeIntrinsicParams(filePath, intrinsicParams, () => {
      ...
      topperTemplateParams.forEach(function(templateParamObject) {
        for (let templateParamElement in templateParamObject) {
          customProfiles.set(
            templateParamElement,
            templateParamObject[templateParamElement]
          );
      ...
      //
      window
        .showInformationMessage("Please select the profile name to apply")
        .then(() => {
          //
          // show the quick-pick selection menu to select the profile
          //
